...
const splledNumbersBackwards = spelledNumbers.map(number => {
...
const findTheNumber = (input, reverse = false) => {
    if(reverse) {
        input = input.split('').reverse().join('');
    }

    var minIndex;
    const firstDigit = input.split('').find((i) => i >= '0' && i <= '9');

    if(firstDigit) {
        minIndex = input.indexOf(firstDigit);
        ans = firstDigit;
    }

    for(const num of reverse ? splledNumbersBackwards : spelledNumbers) {
        if(input.includes(num)) {
            if (ans) {
                ans = input.indexOf(num) < minIndex ? num : ans;
            } else {
                ans = num;
            }
            minIndex = input.indexOf(ans);
        }
    }

    return input.indexOf(ans) === input.indexOf(firstDigit) ? ans : numberMap[reverse ? ans.split('').reverse().join('') : ans];
}
...
inputArray.map(input => {
    const first = findTheNumber(input);
    const last = findTheNumber(input, true);
    const answer = Number(first)*10 + Number(last);
    sum += answer;
})
