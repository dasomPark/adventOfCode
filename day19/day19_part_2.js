const inputData = `qdg{a>3896:kg,s>540:tjk,s>478:A,A}
...
const workflows = inputArray[0].split(`\n`);
const parts = inputArray[1].split(`\n`);

const createWorkflowsMap = (workflows) => {
    const simplifyRule = (rule) => {
        const newRule = rule.slice(0, rule.indexOf(':'));
        const destination = rule.slice(rule.indexOf(':') + 1);
    
        return {rule: newRule, destination};
    }

    const workflowsMap = new Map();
    
    workflows.map(workflow => {
        const key = workflow.slice(0, workflow.indexOf('{'));
        const rulesArray = workflow.slice(workflow.indexOf('{') + 1, workflow.indexOf('}')).split(',');
        const value = {
            rules: [],
        ...
        rulesArray.forEach((rule, index) => {
            if(index !== rulesArray.length - 1) {
                value.rules.push(simplifyRule(rule))
            } else {
                value.defaultDestination = rule;
            }
        })

        workflowsMap.set(key, value);
    })
...
const traverse = (key, rulesArray = []) => {
    if(key === 'R' || key === 'A') {
        if(key === 'A') {
            acceptedRules.push(rulesArray.map(rules => rules.rule))
        }

        return;
    }

    const {rules, defaultDestination} = workflowsMap.get(key);

    const reverseRules = [];
    rules.map(rule => {
        traverse(rule.destination, [...rulesArray, ...reverseRules, rule]);
        reverseRules.push({rule: '!'.concat(rule.rule), destination: rule.destination})
    })

    const newRules = [];
    rules.map(rule => {
        newRules.push({ rule: '!'.concat(rule.rule), destination: rule.destination})
    })
    traverse(defaultDestination, [...rulesArray, ...newRules])
}

const getMinAndMax = (rules) => {
    const minAndMax = {
        x: {
            min: 1,
            max: 4000,
    ...
    rules.map(rule => {
        ...
        if(rule.indexOf('<') !== -1) {
            part = rule.slice(0, rule.indexOf('<'));
            sign = rule.slice(rule.indexOf('<'), rule.indexOf('<') + 1);
            number = Number(rule.slice(rule.indexOf('<') + 1));
            max = number - 1;
        } else {
            part = rule.slice(0, rule.indexOf('>'));
            sign = rule.slice(rule.indexOf('>'), rule.indexOf('>') + 1);
        ...
        if(part.indexOf('!') !== -1) {
            part = part.slice(1);
            if(min < minAndMax[part].max) {
                minAndMax[part].max = min - 1;
            }
    
            if(max > minAndMax[part].min) {
                minAndMax[part].min = max + 1;
            }
        } else {
            if(min > minAndMax[part].min) {
                minAndMax[part].min = min;
            }
    
            if(max < minAndMax[part].max) {
                minAndMax[part].max = max;
            }
        }

    })

    const {x, m, a, s} = minAndMax;
    const total = (x.max - x.min + 1) * (m.max - m.min + 1) * (a.max - a.min + 1) * (s.max - s.min + 1);
...
var ans = 0;
acceptedRules.map(rules => {
    ans += getMinAndMax(rules)
})
