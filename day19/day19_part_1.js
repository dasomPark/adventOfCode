...
const createWorkflowsMap = (workflows) => {
    const simplifyRule = (rule) => {
        ...
        if(rule.indexOf('<') !== -1) {
            ...
            sign = '<';
            number = rule.slice(rule.indexOf('<') + 1, rule.indexOf(':'));
        } else {
            part = rule.slice(0, rule.indexOf('>'));
            sign = '>';
            number = rule.slice(rule.indexOf('>') + 1, rule.indexOf(':'));
...
const simplifyParts = (parts) => {
    const partsArray = [];
    parts.map(part => {
        const numbers = [];
        const partArray = part.slice(1, part.length - 1).split(',')
        partArray.map(partElement => {
            const value = partElement.slice(partElement.indexOf('=') + 1);
            numbers.push(Number(value));
        })
        partsArray.push(numbers);
    })
...
const partsArray = simplifyParts(parts);
const accepted = [];

const findDestination = (rule, parts) => {
    const [x, m, a, s] = parts;
    ...
    if(sign === '<') {
        if(part === 'x' && x < number) {
            return destination;
        } else if(part === 'm' && m < number) {
            return destination;
        } else if(part === 'a' && a < number) {
            return destination;
        } else if(part === 's' && s < number) {
            return destination;
        }
    } else if(sign === '>') {
        if(part === 'x' && x > number) {
    ...
    for(let i = 0; i < partsArray.length; i++) {
        ...
        while(destination !== 'A' && destination !== 'R') {
            ...
            for(let j = 0; j < rules.length; j++) {
                const rule = rules[j];
                const nextDestination = findDestination(rule, parts);
                if(!nextDestination) {
                    if(j === rules.length - 1) {
...
traverse();
var sum = 0;
accepted.map(acceptedArray => {
    acceptedArray.map(ac => {
        sum += ac;
    })
})
