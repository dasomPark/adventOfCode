...
const games = `Game 1: 7 green, 4 blue, 3 red; 4 blue, 10 red, 1 green; 1 blue, 9 red
...
const findTheMatch = (game) => {
    const inputArray = game.split(';');
    const regex = /\d+/;
    const colorCount = {
        'red': 0,
        'green': 0,
    ...
    for(const input of inputArray) {
        ...
        for(const color of colors) {
            const count = Number(color.match(regex)[0]);
            if(color.includes('red')) {
                if(count > maxRed) {
                    return false;
                }
                if(count > colorCount['red']) {
...
const listOfGames = games.split(`\n`);
var sum = 0;

listOfGames.map((game, i) => {
    const gameInput = game.split(':')[1];
    const ans = findTheMatch(gameInput);
    if (ans) {
        sum += i+1;
    }
})
