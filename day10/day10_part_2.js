const input = `-7-|-J-F7F--7.|77.77FFJ7F77..7FJ-77-FF7FF-777F-77--L7FFJJF77FF|7F7F--FJ7-7-77-7-7.F-7--F7.F7F---7F7FF7FL|7-7-F--F7-FF7-|.|77LL-FFL7J7F-F-F7.
...
const mazeArray = smallInput.split(`\n`).map(pipes => pipes.split(''));
var mazeArrayCopy = mazeArray.map(arr => arr.slice());
const maxWidthIndex = mazeArray[0].length - 1;
const maxHeightIndex = mazeArray.length - 1;

const findStartingPosition = (arrays) => {
...
const findNextPipe = (index1, index2, root = '', first = false) => {
    const ans = [];
    if(first) {
        // top
        if(index1 - 1 >= 0) {
            const top = mazeArray[index1 - 1][index2];
            if(top === "|" || top === '7' || top === 'F') {
                ans.push([index1 - 1, index2, 'bottom'])
            }            
        }
        // bottom
        if(index1 + 1 <= maxHeightIndex) {
            const bottom = mazeArray[index1 + 1][index2];
            if(bottom === '|' || bottom === 'L' || bottom === 'J') {
                ans.push([index1 + 1, index2, 'top'])
            }            
        }
        // right
        if(index2 + 1 <= maxWidthIndex) {
            const right = mazeArray[index1][index2 + 1];
            if(right === '-' || right === 'J' || right === '7') {
                ans.push([index1, index2 + 1, 'left'])
            }
        }
        // left
        if(index2 - 1 >= 0) {
            const left = mazeArray[index1][index2 - 1];
            if(left === '-' ||left === 'L' || left === 'F') {
                ans.push([index1, index2 - 1, 'right'])
            }
        }
    } else {
        const current = mazeArray[index1][index2];
        if(current === '|') {
            if(root === 'top') {
                // top -> bottom
                ans.push([index1 + 1, index2, 'top']);
    ...
    const current = mazeArray[index1][index2];
    if(current === "|" || current === "F" || current === 'J') {
        mazeArrayCopy[index1][index2] = "|";
    } else if(current === '-') {
        mazeArrayCopy[index1][index2] = "";
    }
    if(first) {
        mazeArrayCopy[index1][index2] = "F";
        // mazeArrayCopy[index1][index2] = "J";
    }
...
const startingPosition = findStartingPosition(mazeArray);
const startingPipes = findNextPipe(...startingPosition, '' ,true);
var firstPipe = startingPipes[0];
var secondPipe = startingPipes[1];
var count = 1;

while((firstPipe[0] !== secondPipe[0]) || (firstPipe[1] !== secondPipe[1])) {
    firstPipe = findNextPipe(...firstPipe)[0];
    secondPipe = findNextPipe(...secondPipe)[0];
    count++;
}


mazeArrayCopy.map(arr => {
    console.log(arr.join(''))
})

mazeArrayCopy = mazeArrayCopy.map(arr => {
    var newArr = arr.join('')
    newArr = newArr.replaceAll('L7', '|');
    newArr = newArr.replaceAll('FJ', '|');
    newArr = newArr.replaceAll('LJ', '');
    newArr = newArr.replaceAll('F7', '')
    newArr = newArr.replaceAll('-', '')

    return newArr.split('');
})

for(var i = 0; i <= mazeArrayCopy.length - 1; i++) {
    console.log(mazeArrayCopy[i])
    for(var j = 0; j <= mazeArrayCopy[i].length - 1; j++) {
        var count = 0;
        for(var k = j + 1; k <= mazeArrayCopy[i].length - 1; k++) {
            if(mazeArrayCopy[i][j] !== '.') {
                break;
            }
            count++;
        }
