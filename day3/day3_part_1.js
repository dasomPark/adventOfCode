...
const findNumbers = (str) => {
    const regex = /\d+/g;
    const numbers = [...str.matchAll(regex)];
    numbers.forEach((number, index) => {
        number.endIndex = number[0].length + number.index - 1;
    })
    return numbers;
}

const checkAdjacentRows = (number, line) => {
    const startIndex = number.index > 0 ? number.index - 1 : number.index;
    const endIndex = number.endIndex < number.input.length - 1 ? number.endIndex + 1 : number.endIndex;
    const highlight = line.substring(startIndex, endIndex+1);
    const hasEngine = highlight.split('').some((char) => char !== '.' && !Number(char));

    return hasEngine;
}

const compareLines = (line, upperLine = '', lowerLine = '') => {
    const numbers = findNumbers(line);
    var sum = 0;
    numbers.forEach((number) => {
        if(!upperLine) {
            const hasEngine = checkAdjacentRows(number, lowerLine);
            if(hasEngine) {
                sum = sum + Number(number[0]);
            }
        } else if(!lowerLine) {
            const hasEngine = checkAdjacentRows(number, upperLine);
            if(hasEngine) {
                sum = sum + Number(number[0]);
            }
        } else {
            ...
            if (!hasEngine) {
                const hasEngine2 = checkAdjacentRows(number, upperLine);
                if(hasEngine2) {
                    sum = sum + Number(number[0]);
                } else {
                    // Check adjacent columns
                    if(number.index > 0) {
                        const target = number.input[number.index-1]
                        if(target !== '.' && !Number(target)) {
                            sum = sum + Number(number[0]);
                        }
                    }

                    if(number.endIndex < number.input.length - 1) {
                        const target = number.input[number.endIndex+1]
                        if(target !== '.' && !Number(target)) {
                            sum = sum + Number(number[0]);
                        }
                    }
...
for(const [index, input] of inputArray.entries()) {
    if(index == 0) {
        const ans = compareLines(inputArray[index], '', inputArray[index+1]);
        sum = ans + sum;
    } else if (index == inputArray.length - 1) {
        const ans = compareLines(inputArray[index], inputArray[index-1], '');
        sum = ans + sum;
    } else {
        var ans = compareLines(inputArray[index], inputArray[index-1], inputArray[index+1]);
        sum = ans + sum;
    }
}
