...
const compareLines = (line, upperLine = '', lowerLine = '') => {
    const stars = findStarGears(line);
    var totalSum = 0;
    stars.forEach(star => {
        var count = 0;
        var sum = 1;
        ...
        if(star.index > 0) {
            ...
            numbers.forEach(num => {
                if(num.endIndex === star.index - 1) {
                    count = count + 1;
                    matchList.push(Number(num[0]))
                }
            })
        }
        // check right
        if(star.index < star.input.length - 1) {
            const numbers = findNumbers(line);
            numbers.forEach(num => {
                if(num.index === star.index + 1) {
                    count = count + 1;
                    matchList.push(Number(num[0]))
                }
            })
        ...
        // check upper line
        if(upperLine) {
            const numbers = findNumbers(upperLine);
            numbers.forEach(number => {
                if(number.endIndex >= star.index - 1 && number.endIndex <= star.index + 1) {
                    count = count + 1;
                    matchList.push(Number(number[0]))
                } else if (number.index >= star.index - 1 && number.index <= star.index + 1) {
                    count = count + 1;
                    matchList.push(Number(number[0]))
                }
            })
        ...
        // check lower line
        if(lowerLine) {
            findNumbers(lowerLine).forEach(number => {
                if(number.endIndex >= star.index - 1 && number.endIndex <= star.index + 1) {
                    count = count + 1;
                    matchList.push(Number(number[0]))
                } else if (number.index >= star.index - 1 && number.index <= star.index + 1) {
        ...
        if(matchList.length == 2) {
            totalSum = totalSum + (matchList[0] * matchList[1]);
        }
