const input = `seeds: 2906961955 52237479 1600322402 372221628 2347782594 164705568 541904540 89745770 126821306 192539923 3411274151 496169308 919015581 8667739 654599767 160781040 3945616935 85197451 999146581 344584779
...
const simplifyMaps = (str) => {
    const numberArray = str.replace(/.+:\s+/g, '').split(/\s+/).map(num => Number(num));
    var listOfArrays = [];
    var subArray = [];
    var ans = [];
...
const simplifySeeds = (str) => {
    const seeds = str.replace(/.+:\s+/g, '').split(/\s+/).map(num => Number(num));
    const newSeeds = [];
